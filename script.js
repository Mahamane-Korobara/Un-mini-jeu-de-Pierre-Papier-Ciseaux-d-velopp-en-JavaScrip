...
function nombreDeClique() {
    ...
    paragraphe1.innerHTML = `Pour le tour ${boutonClique.length} : ${resultat}`;
    contentantTourDeClique.appendChild(paragraphe1);

    // Si la limite de 5 tours est atteinte, afficher les résultats finaux
    if (boutonClique.length >= 5) {
        afficherResultatsFinal();
        // Désactiver les boutons
        choixPossibles.forEach(bouton => bouton.disabled = true);
    }
}
...
function afficherResultatsFinal() {
    ...
    // creation d'un bouton pour reinitialiser le jeu 
    const resetButton = document.createElement('button');
    resetButton.id = 'resetButton';
    resetButton.textContent = 'Réjouer';
    contenantResulatFinal.appendChild(resetButton);
    // affichage du bouton pour la reinitialisation 
    resetButton.style.display = 'block';
    //ajout d'un evenement pour la reinitialisation de la page 
